...
<Card title="⚠️ Page obsolète">
	**Cette page est obsolète et doit être mise à jour.**
</Card>

Le modèle principal contient la structure HTML de la page et l'ensemble des éléments apparaissant sur tout votre site
Biblys (en-tête, menu, pied de page, etc.)

C'est le seul modèle à ne pas être au format Twig. Il s'agit d'un simple HTML, avec toutefois des variables spécifiques
qu'il faut absolument inclure pour permettre le bon fonctionnement de Biblys.
...
* `{ANALYTICS_TRACKER}` : permet d'insérer le code de suivi Google Analytics si l'option de site `google-analytics-id`
  est définie
